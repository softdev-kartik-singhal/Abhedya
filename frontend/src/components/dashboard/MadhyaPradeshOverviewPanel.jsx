...
// ─────────────────────────────────────────────────────────────────────────────
const MadhyaPradeshOverviewPanel = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch("/madhya-pradesh-state.geojson").then(res => res.json()),
      fetch("/madhya-pradesh-districts.geojson").then(res => res.json())
    ]).then(([stateData, districtsData]) => {
      if (!isMounted || !mapRef.current) return;
      // 1. World mask (strongly fades all other states outside Madhya Pradesh)
      ...
      // 3. Madhya Pradesh State Fill Highlight (spotlight effect)
      L.geoJSON(stateData, {
        style: {
          fillColor: "#ffffff",
          fillOpacity: 0.22,
  ...
  useEffect(() => {
    ...
    if (showClusters) {
      ...
      incidents.forEach((inc) => {
        const dName = inc.district || "Bhopal";
      ...
      Object.entries(clusters).forEach(([district, data]) => {
        const districtIncs = incidents.filter(i => (i.district || "Bhopal") === district);
        const categories = {};
...
export default MadhyaPradeshOverviewPanel;
