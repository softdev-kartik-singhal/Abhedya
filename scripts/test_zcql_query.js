...
async function main() {
    ...
    const zcqlPayload = JSON.stringify({ query: "SELECT * FROM Employee" });

    console.log("Executing ZCQL query: SELECT * FROM Employee ...");
    const res = await makeApiRequest({
        hostname: "api.catalyst.zoho.in",
        port: 443,
        path: `/baas/v1/project/${projectId}/zcql`,
        method: "POST",
        headers: {
            ...baseHeaders,
            "Content-Type": "application/json",
            "Content-Length": Buffer.byteLength(zcqlPayload)
        }
    }, zcqlPayload);

    console.log("ZCQL Status:", res.status);
    console.log("ZCQL Response:", res.body);
}
