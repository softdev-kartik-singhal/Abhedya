...
if (localCreds) {
    ...
    async function testZcql() {
        ...
        console.log('Testing ZCQL Query: SELECT * FROM State...');
        try {
            const queryResult = await app.zcql().executeZCQLQuery('SELECT * FROM State');
            console.log('ZCQL QUERY SUCCESS!');
            console.log('Results:', JSON.stringify(queryResult, null, 2));
        } catch (err) {
            console.error('ZCQL QUERY FAILED!');
