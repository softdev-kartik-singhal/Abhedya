...
async function main() {
    ...
    const res = await makeApiRequest({
        ...
        port: 443,
        path: `/baas/v1/project/${projectId}/table/CrimeSubHead/row`,
        method: "POST",
        headers: {
            ...baseHeaders,
    ...
    console.log("CrimeSubHead Insert Status:", res.status);
    console.log("CrimeSubHead Insert Response:", res.body);

    const parsed = JSON.parse(res.body);
    const subheadRowId = parsed.data && parsed.data[0] ? parsed.data[0].ROWID : null;

    const rowIdsPath = path.join(__dirname, 'rowid_mapping.json');
    const existing = JSON.parse(fs.readFileSync(rowIdsPath, 'utf-8'));
    existing.CrimeSubHead = subheadRowId;
    fs.writeFileSync(rowIdsPath, JSON.stringify(existing, null, 2));
    console.log("Updated rowid_mapping.json with CrimeSubHead ROWID:", subheadRowId);
}
