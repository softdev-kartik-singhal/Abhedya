...
async function main() {
    ...
    console.log("--- 1. Orgs ---");
    const orgs = await makeApiRequest({ hostname: "api.catalyst.zoho.in", port: 443, path: "/baas/v1/orgs", method: "GET", headers: baseHeaders });
    console.log(orgs.body);

    console.log("\n--- 2. Project Details ---");
    const prj = await makeApiRequest({ hostname: "api.catalyst.zoho.in", port: 443, path: `/baas/v1/project/${projectId}`, method: "GET", headers: baseHeaders });
    console.log(prj.body);

    console.log("\n--- 3. Environment Details ---");
    const env = await makeApiRequest({ hostname: "api.catalyst.zoho.in", port: 443, path: `/baas/v1/project/${projectId}/environment`, method: "GET", headers: baseHeaders });
    console.log(env.body);

    console.log("\n--- 4. All Employee Rows in Datastore ---");
    const empRows = await makeApiRequest({ hostname: "api.catalyst.zoho.in", port: 443, path: `/baas/v1/project/${projectId}/table/Employee/row`, method: "GET", headers: baseHeaders });
    console.log(empRows.body);
}
