...
async function main() {
    ...
    console.log("--- Testing Environment Header Variants ---");

    const envs = ["Development", "Production"];
    for (const envName of envs) {
        console.log(`\nQuerying environment: ${envName}`);
        const res = await makeApiRequest({
            ...
            headers: {
                ...baseHeaders,
                "CATALYST-ENV": envName
            }
        });
        console.log(`Status (${envName}):`, res.status);
        console.log(`Response (${envName}):`, res.body);
    }
}
