...
async function testInsert() {
    ...
    console.log("4. Handshake GET /baas/v1/project/" + projectId + "/environment...");
    ...
    console.log("5. Testing POST row insert to /table/State/row...");
    const rowPayload = JSON.stringify([
        { "StatelD": 1, "StateName": "Karnataka", "NationalitylD": 1, "Active": true }
    ]);

    const res = await makeApiRequest({
        hostname: "api.catalyst.zoho.in",
        port: 443,
        path: `/baas/v1/project/${projectId}/table/State/row`,
        method: "POST",
        headers: {
            ...baseHeaders,
            "Content-Type": "application/json",
            "Content-Length": Buffer.byteLength(rowPayload)
        }
    }, rowPayload);

    console.log("Insert Response Status:", res.status);
    console.log("Insert Response Body:", res.body);
}
