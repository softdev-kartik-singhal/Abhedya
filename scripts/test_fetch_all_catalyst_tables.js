...
async function main() {
    ...
    const tables = [
        "CaseMaster", "District", "Unit", "Employee", "CaseCategory", 
        "GravityOffence", "CaseStatusMaster", "Court", "CrimeHead", 
        "ComplainantDetails", "Accused"
    ];

    console.log("=== Fetching Data Views for all Tables in Development Environment ===");
    for (const t of tables) {
        const res = await makeApiRequest({
            hostname: "api.catalyst.zoho.in",
            port: 443,
            path: `/baas/v1/project/${projectId}/table/${t}/row`,
            method: "GET",
            headers: baseHeaders
        });
        try {
            const data = JSON.parse(res.body);
            console.log(`\nTable [${t}] -> Status: ${res.status}, Count: ${data.data ? data.data.length : 0}`);
            if (data.data && data.data.length > 0) {
                console.log("Sample Rows:", JSON.stringify(data.data.slice(0, 3), null, 2));
            }
        } catch(e) {
            console.log(`Table [${t}] -> Error: ${res.body}`);
        }
    }
}
