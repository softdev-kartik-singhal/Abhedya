...
async function main() {
    ...
    const rankRowId = await insertOrGetRowId("Rank", { RankID: 1, RankName: "PSI (Police Sub-Inspector)", Active: true });
    const designationRowId = await insertOrGetRowId("Designation", { DesignationID: 1, DesignationName: "Station House Officer", Active: true });

    existing.Rank = rankRowId;
    existing.Designation = designationRowId;

    fs.writeFileSync(rowIdsPath, JSON.stringify(existing, null, 2));
    console.log("Updated rowid_mapping.json with Rank and Designation ROWIDs.");
}
