...
async function main() {
    ...
    const rowIds = JSON.parse(fs.readFileSync(path.join(__dirname, 'rowid_mapping.json'), 'utf-8'));

    const caseMasterPayload = [{
        CrimeNo: "FIR/2026/DEVFORM1",
        CaseNo: "202677881",
        CrimeRegisteredDate: "2026-07-22",
        PolicePersonID: String(rowIds.Employee),
        PoliceStationID: String(rowIds.Unit),
        CaseCategoryID: String(rowIds.CaseCategory),
        GravityOffenceID: String(rowIds.GravityOffence),
        CrimeMajorHeadID: String(rowIds.CrimeHead),
        CaseStatusID: String(rowIds.CaseStatusMaster),
        CourtID: String(rowIds.Court),
        IncidentFromDate: "2026-07-22 10:00:00",
        IncidentToDate: "2026-07-22 11:30:00",
        InfoReceivedPSDate: "2026-07-22 12:00:00",
        latiutude: 12.9716,
        longitude: 77.5946,
        BriefFacts: "FIR registered via portal form in Development environment"
    }];

    const payloadStr = JSON.stringify(caseMasterPayload);

    console.log("Inserting CaseMaster into Development Environment...");
    ...
    console.log("CaseMaster Development Insert Status:", res.status);
    console.log("CaseMaster Development Insert Response:", res.body);
}
