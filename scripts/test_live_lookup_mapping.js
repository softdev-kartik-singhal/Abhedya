const path = require('path');
const fs = require('fs');
const os = require('os');
const crypto = require('crypto');
const https = require('https');

function tryGetLocalCliCredentials() {
    const homedir = os.homedir();
    const baseDir = path.join(homedir, 'Library/Preferences/zcatalyst-cli-nodejs');
    const keyPath = path.join(baseDir, '.zcatalyst-cli-key');
    const configPath = path.join(baseDir, 'zcatalyst-cli-v1.json');
    
    const encryptionKey = fs.readFileSync(keyPath);
    const configRaw = fs.readFileSync(configPath, 'utf-8');
    const config = JSON.parse(configRaw);
    
    const activeDc = config.active_dc || 'us';
    const dcConfig = config[activeDc];
    const encrypted = dcConfig.credential;
    const data = Buffer.from(encrypted, 'hex');
    const initializationVector = data.slice(0, 12);
    const authTag = data.slice(13, 29);
    const cipherText = data.slice(29);
    
    const derivedKey = crypto.pbkdf2Sync(encryptionKey, initializationVector, 100000, 32, 'sha512');
    const decipher = crypto.createDecipheriv('aes-256-gcm', derivedKey, initializationVector);
    decipher.setAuthTag(authTag);
    const decrypted = Buffer.concat([decipher.update(cipherText), decipher.final()]);
    const credObj = JSON.parse(decrypted.toString());
    
    return {
        dc: activeDc,
        clientId: '1000.D5IIHDXSPN2MII26AD0V61I6RMVSNM',
        clientSecret: '02ee875ecfc50573e5cc8d62916ad3077be20d0f42',
        refreshToken: credObj.token.slice(2)
    };
}

async function getFreshAccessToken() {
    const creds = tryGetLocalCliCredentials();
    const bodyData = `client_id=${creds.clientId}&client_secret=${creds.clientSecret}&refresh_token=${creds.refreshToken}&grant_type=refresh_token`;
    
    return new Promise((resolve, reject) => {
        const options = {
            hostname: `accounts.zoho.${creds.dc}`,
            port: 443,
            path: '/oauth/v2/token',
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
                'Content-Length': Buffer.byteLength(bodyData)
            }
        };
        const req = https.request(options, (res) => {
            let body = '';
            res.on('data', chunk => body += chunk);
            res.on('end', () => {
                try {
                    const parsed = JSON.parse(body);
                    if (parsed.access_token) resolve(parsed.access_token);
                    else reject(new Error(body));
                } catch(e) { reject(e); }
            });
        });
        req.on('error', reject);
        req.write(bodyData);
        req.end();
    });
}

function makeApiRequest(options) {
    return new Promise((resolve, reject) => {
        const req = https.request(options, (res) => {
            let body = '';
            res.on('data', chunk => body += chunk);
            res.on('end', () => resolve({ status: res.statusCode, headers: res.headers, body }));
        });
        req.on('error', reject);
        req.end();
    });
}

async function main() {
    const token = await getFreshAccessToken();
    const projectId = "56116000000017001";
    const orgId = "60077759371";
    const baseHeaders = {
        "Authorization": `Zoho-oauthtoken ${token}`,
        "Accept": "application/vnd.catalyst.v2+json",
        "CATALYST-ORG": orgId,
        "environment": "Development",
        "User-Agent": "zcatalyst-cli/1.27.0"
    };

    await makeApiRequest({ hostname: "api.catalyst.zoho.in", port: 443, path: "/baas/v1/orgs", method: "GET", headers: baseHeaders });
    await makeApiRequest({ hostname: "api.catalyst.zoho.in", port: 443, path: `/baas/v1/project/${projectId}`, method: "GET", headers: baseHeaders });
    await makeApiRequest({ hostname: "api.catalyst.zoho.in", port: 443, path: `/baas/v1/project/${projectId}/environment`, method: "GET", headers: baseHeaders });

    const fetchRows = async (table) => {
        const res = await makeApiRequest({
            hostname: "api.catalyst.zoho.in",
            port: 443,
            path: `/baas/v1/project/${projectId}/table/${table}/row`,
            method: "GET",
            headers: baseHeaders
        });
        try {
            const data = JSON.parse(res.body);
            return data.data || [];
        } catch(e) {
            return [];
        }
    };

    const cases = await fetchRows("CaseMaster");
    const units = await fetchRows("Unit");
    const districts = await fetchRows("District");
    const employees = await fetchRows("Employee");
    const categories = await fetchRows("CaseCategory");
    const gravity = await fetchRows("GravityOffence");
    const statuses = await fetchRows("CaseStatusMaster");
    const crimeHeads = await fetchRows("CrimeHead");
    const complainants = await fetchRows("ComplainantDetails");
    const accused = await fetchRows("Accused");

    const unitMap = {}; units.forEach(u => unitMap[u.ROWID] = u.UnitName);
    const distMap = {}; districts.forEach(d => distMap[d.ROWID] = d.DistrictName);
    const empMap = {}; employees.forEach(e => empMap[e.ROWID] = e);
    const catMap = {}; categories.forEach(c => catMap[c.ROWID] = c.LookupValue);
    const gravMap = {}; gravity.forEach(g => gravMap[g.ROWID] = g.LookupValue);
    const statusMap = {}; statuses.forEach(s => statusMap[s.ROWID] = s.CaseStatusName);
    const headMap = {}; crimeHeads.forEach(h => headMap[h.ROWID] = h.CrimeGroupName);

    const compMap = {}; complainants.forEach(c => { if (c.CaseMasterID) compMap[c.CaseMasterID] = c.ComplainantName; });
    const accMap = {}; accused.forEach(a => { if (a.CaseMasterID) accMap[a.CaseMasterID] = a.AccusedName; });

    console.log(`\nNormalized ${cases.length} Live CaseMaster Rows with Online Catalogs:`);

    const normalized = cases.map(c => {
        const emp = empMap[c.PolicePersonID] || {};
        return {
            ROWID: c.ROWID,
            crimeNo: c.CrimeNo,
            caseNo: c.CaseNo,
            regDate: c.CrimeRegisteredDate,
            unit: unitMap[c.PoliceStationID] || "Central Police Station",
            district: distMap[c.DistrictID] || "Bengaluru City",
            crimeHead: catMap[c.CaseCategoryID] || headMap[c.CrimeMajorHeadID] || "Property Offences",
            severity: gravMap[c.GravityOffenceID] || "MEDIUM",
            status: statusMap[c.CaseStatusID] || "Under Investigation",
            complainantName: compMap[c.ROWID] || c.complainantName || "Citizen Complainant",
            allottedOfficerName: emp.FirstName || "PSI Investigating Officer",
            allottedOfficerKgid: emp.KGID || "KSP-2026-1001",
            accusedName: accMap[c.ROWID] || c.accusedName || "Unidentified Suspect",
            briefFacts: c.BriefFacts
        };
    });

    console.log(JSON.stringify(normalized.slice(0, 5), null, 2));
}

main().catch(console.error);
