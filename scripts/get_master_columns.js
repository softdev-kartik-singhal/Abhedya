...
async function getColumns(tableName, headers, projectId) {
    const res = await makeApiRequest({
        hostname: "api.catalyst.zoho.in",
        port: 443,
        path: `/baas/v1/project/${projectId}/table/${tableName}/column`,
        method: "GET",
        headers: headers
    });
    try {
        ...
        if (data && data.data) {
            data.data.forEach(col => {
                console.log(`  - ${col.column_name} (${col.data_type}) [Mandatory: ${col.is_mandatory}]`);
            });
        } else {
            console.log(res.body);
        }
    } catch(e) { console.error(res.body); }
}
...
async function main() {
    ...
    const tables = ["Employee", "Unit", "CaseCategory", "GravityOffence", "CrimeGroupMaster", "CrimeHeadMaster", "CaseStatusMaster", "Court"];
    for (const t of tables) {
        await getColumns(t, baseHeaders, projectId);
    }
}
